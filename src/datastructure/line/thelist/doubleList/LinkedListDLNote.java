...
import datastructure.line.thelist.doubleList.Iterator.Iterator;
import datastructure.line.thelist.doubleList.Iterator.LinkedListIterator;



public class LinkedListDLNote implements datastructure.line.thelist.doubleList.LinkedList{
	...
	protected DLNode checkPosition(Node p) throws InvalidNodeException {
		if(p == null) throw new InvalidNodeException("错误，p 为空");
		
		if(p == head) throw new InvalidNodeException("错误, p 指向头节点, 非法");
		
		if(p == tail) throw new InvalidNodeException("错误, p 指向尾节点, 非法");
	...
	public Node first() throws OutOfBoundaryException {
		...
		if(isEmpty()) throw new OutOfBoundaryException("错误，链表为空");
	...
	@Override
	public Node getNext(Node p) throws OutOfBoundaryException,
			InvalidNodeException {
		
		DLNode node = checkPosition(p);
	
		if(node == tail) throw new OutOfBoundaryException("错误，已经到链表最后");
	...
	@Override
	public Node getPre(Node p) throws OutOfBoundaryException,
			InvalidNodeException {
		
		DLNode node = checkPosition(p);
	
		if(node == head) throw new OutOfBoundaryException("错误，已经到链表最后");
