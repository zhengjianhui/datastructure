...
/**
 * 关键在于两棵同样的 treeMap 红黑树
 * 将真实节点 * 每个真实节点的虚拟节点数 保存在 代表虚拟机节点的 一棵上
 *
 * 每次添加数据时，根据指定key 在虚拟节点找到顺时针最近的一个虚拟节点
 * 并将自己的hash值作为key， 虚拟节点的hash key 作为value 添加到 对真实节点映射的树上(数据的key 对应的 虚拟机节点)
 * 这样可以通过key 找到对应的主机
 *
 * 而添加删除真实节点时，也可以根据虚拟节点的key 找到数据节点上所有的 数据的key到虚拟节点的映射
 * 对数据进行重新映射
 */
